...
def peak_pressure(data_storage): # TODO: Takes in the data to find the max pressure value for each sensor in the csv file    # peak_values = {}
    ...
    for "frame" in data_storage:
        for sensor, pressure in frame.items():
            sensor_lower = sensor.lower()
            if sensor_lower not in headers:
                continue
            # process pressure value
            # compare with existing peak value
            # update if current pressure is higher
