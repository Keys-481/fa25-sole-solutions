# src/sole_solutions/core/export_manager.py
    ...
    try:
        with open(path, "w", newline="", encoding="utf-8") as f:
            ...
            for i in range(n):
                ...
                w.writerow([i, f"{ap:.6f}" if ap != "" else "", f"{vg:.6f}" if vg != "" else ""])
        return None
...
def export_summary_docx(summary: Dict, path: str) -> Optional[str]:
    ...
    try:
        ...
        for k in ("frames","sensors","global_min","global_max",
                  "contact_time_frames","contact_threshold","pti","dt"):
            add_line(k)
        ...
        hdr[0].text = "Frame"; hdr[1].text = "Avg Pressure"; hdr[2].text = "Estimated vGRF"
...
def export_summary_pdf(summary: Dict, path: str) -> Optional[str]:
    ...
    try:
        ...
        c.drawString(1*inch, y, "Sole Solutions Report"); y -= 0.4*inch
        c.setFont("Helvetica", 11); c.drawString(1*inch, y, "Session Summary"); y -= 0.25*inch

        def line(txt):
            nonlocal y
            c.drawString(1*inch, y, txt); y -= 0.2*inch
            if y < 1*inch:
